...
- [ ] **GTM phone-conversion tag (configure in Scott's GTM dashboard, no code change here).** The new site fires a `phone_call_click` dataLayer event on every `tel:` click in header/footer/sticky/quote/contact/locations. To wire it to a Google Ads phone-call conversion: in the GTM-ND38S235 container — (1) New Trigger: Custom Event, event name `phone_call_click`. (2) New Tag: Google Ads Conversion Tracking with Scott's existing AW-XXXXXXXXX conversion ID and the phone-call conversion label. (3) Set the trigger to the event from step 1. (4) Submit + publish. dataLayer payload includes `phone_number`, `call_location` (header / footer / sticky_desktop / sticky_mobile / quote_page / inline), and `page_path` so GTM can split conversions by surface.
